...
describe('jms - repeating element attribute equals current date', function() {
  ...
  it('should report a mismatch where the actual repeating group element value does not match the expected value', function() {
    ...
    var expectedMessage = [
      {repeatingGroup: {path: 'thingContainingRepeatingGroups', repeater: 'RepeatingGroup', number: 1}, path: 'fieldOneOfRepeatingGroup', attribute: 'attribute1', equals: currentLocalDateRegexPattern, dateFormat: 'YYYY-MM-DD'},
      {repeatingGroup: {path: 'thingContainingRepeatingGroups', repeater: 'RepeatingGroup', number: 2}, path: 'fieldOneOfRepeatingGroup', attribute: 'attribute1', equals: currentUtcDateRegexPattern, dateFormat: 'DD MM-YYYY'}
    ];
    ...
    assert.deepEqual(result.checks[2], {
      ...
      pass: false,
      path: 'thingContainingRepeatingGroups.RepeatingGroup.fieldOneOfRepeatingGroup number: 1 (attribute: attribute1)'
    });
    ...
    assert.deepEqual(result.checks[4], {
      ...
      pass: false,
      path: 'thingContainingRepeatingGroups.RepeatingGroup.fieldOneOfRepeatingGroup number: 2 (attribute: attribute1)'
    });
  });
